...
public class FettuccineServices {
    ...
    private boolean initRpc() {
        if(shellConnection.isConnected()) {
            if(!initComplete) {
                remoteClient = new RemoteClient(shellConnection.getFileManager(),
                        FettuccineConstants.SYS_DIR,FettuccineConstants.FETTUCCINE_RPC_VERSION);
    ...
    public void mount(     @ShellOption({"-S", "--server"}) String server,
