...
public class Demo {
  ...
  public static void parallelRun(String host, String msg) {
    class runner implements Runnable {
      ...
      public void run() {
        RemoteClient client =
            new RemoteClient(hostName, 9092, serviceName, Agnolotti.DEFAULT_VER, serviceName);
    ...
    System.out.println(
        "Total milliseconds "
            ...
            + Double.toString(executedInvokes / (totalTime / 1000)));
    System.out.println(
        "Effetive Cost: " + Long.toString(totalTime / executedInvokes) + " ms per call");
  }
  ...
  public static void nullOrRspTest(TestService testService, boolean test, String echoMsg) {
    ...
    System.out.println(
        "Performance: " + Long.toString(callPerSec) + " Invocations per second on a single client");
  ...
  public static void clientDemo(String host, String msg) {
    ...
    RemoteClient client =
        new RemoteClient(host, 9092, serviceName, Agnolotti.DEFAULT_VER, serviceName);
    TestService testService = (TestService) client.getRemoteService(TestService.class);
  ...
  public static void serverDemo() {
    ServiceManager serviceManager =
        new ServiceManager(Agnolotti.DEFAULT_VER, serviceName, 9092, serviceName, serviceName);
