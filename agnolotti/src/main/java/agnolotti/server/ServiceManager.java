...
public class ServiceManager {
  ...
  public ServiceManager(
      GelatoServerManager manager, String serviceName, String userName, String version) {
    this(manager, manager.getConnection(), manager.getRoot(), serviceName, userName, version);
  }

  public ServiceManager(
      GelatoServerManager manager,
    ...
    root = location;
    this.serviceName =
        new ServiceGenericDirectory(
            userName,
            userGroup,
            serveletManager.getDescriptorManager().generateDescriptor().getDescriptorId(),
            serviceName,
            serveletManager);
    this.serviceVersion =
        new ServiceGenericDirectory(
  ...
  public ServiceManager(
    ...
    root =
        new ServiceGenericDirectory(
            userName, userGroup, Agnolotti.ROOT_ID, Agnolotti.ROOT_NAME, serveletManager);
    serviceName =
        new ServiceGenericDirectory(
            userName, userGroup, Agnolotti.NAME_ID, nameSpace, serveletManager);
    serviceVersion =
        new ServiceGenericDirectory(
            userName, userGroup, Agnolotti.VER_ID, version, serveletManager);
